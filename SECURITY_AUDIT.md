...
| Partiellement traité — exploitant plateforme | Réduire les expositions F1 et séparer le compte d'exécution partagé `codex-deploy` | F1 est lié à `127.0.0.1:5000` et les règles UFW IPv4/IPv6 `5000` ont été retirées et vérifiées le 2026-09-22. Le compte backend possède encore releases et configuration ; sa séparation reste à planifier. |
| Traité — exploitant plateforme | Durcir SSH et limiter les privilèges de déploiement | Root, mots de passe, X11 et tous les forwardings sont refusés depuis le 2026-09-22. La clé `jrechau` est validée, l'ancien tunnel PostgreSQL n'est plus permis, `codex-deploy` est restreint et son `NOPASSWD: ALL` a été remplacé par une allowlist. Les sauvegardes et restaurations utilisent des helpers root-owned hors des releases. |
